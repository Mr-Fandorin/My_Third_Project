...
file_handler = logging.FileHandler('../logs/mask.log', encoding="utf-8", mode='w')
...
def get_mask_card_number(card_number: int) -> str:
    ...
    if len(str_card_number) == 16:
        logger.info(f'Маскируем часть номера карты звездочками')
        ...
        logger.info(f'Нет номера карты')
        return "нет номера карты"
    else:
        logger.info(f'Неверный номер карты')
...
def get_mask_account(account_number: int) -> str:
    ...
    if len(str_account_number) == 20:
        logger.info(f'Маскируем часть номера счета звездочками')
        return f"**{str_account_number[-4:]}"
    elif len(str_account_number) == 0:
        logger.info(f'Нет номера счета')
        return "нет номера счета"
    else:
        logger.info(f'Неверный номер счета')
